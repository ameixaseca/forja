...
Exportação integral de dados do usuário (RF-091), a única capacidade LGPD que precisa da API — `purchase_receipts` não tem nenhuma política de RLS para `authenticated`, então é inacessível ao cliente fora de `service_role` (ADR-0011). Exclusão de conta (RF-092) não está aqui: `deletion_requests` já tem RLS de insert para o próprio usuário, e o expurgo roda como rotina `security definer` agendada no banco, sem precisar de código de API.
