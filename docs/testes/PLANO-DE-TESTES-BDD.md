...
**Estilo declarativo, não imperativo.** O cenário descreve a regra, não a sequência de toques. `Quando o ciclo é encerrado como cumprido` — não `Quando o usuário toca em Encerrar e confirma no diálogo`. A exceção é onde o requisito _é_ sobre a interface: RF-010 (20 segundos), RF-062 (pré-visualização), RF-064 (camadas desligadas).
